...
def subThreadIn(myconnection, connNumber):
    ...
    while True:
        try:
            ...
            if recvedMsg:
                # 调整输出格式
                print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),mydict[connNumber], ':', recvedMsg)
                # 转发给所有人
                tellOthers(connNumber, datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" >>>> " + mydict[connNumber] + ' :' + recvedMsg)
